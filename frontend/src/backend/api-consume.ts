...
export const fetchSingleProject = async (projectId: number): Promise<Project> => {
    return await DefaultService.readProjectProjectProjectIdGet(projectId)
}

export const createReview = async (projectId: number, review: ReviewCreate): Promise<Review> => {
    return await DefaultService.createReviewProjectProjectIdCreateReviewPost(projectId, review)
}
